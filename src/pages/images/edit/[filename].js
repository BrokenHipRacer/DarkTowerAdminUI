...
export default class ImageDetails extends Component {
   ...
   * @param {Object} res
   * @param {Object} query
   * @return {Promise<{filename: *, fileSize: *, fileCreated: *, notFoundError: *}>}
   */
    ...
    if (!this.state.filenameInputValue) {
      ...
      checkIfImageFilenameExists(this.state.filenameInputValue, function(existsResponse) {
        if (!existsResponse.success) {
          self.setState({
            updateSubmitError: false,
            filenameAlreadyExistsError: true,
            updateSuccess: false,
            updateLoading: false});
        } else {
          updateImageFilename(
              self.props.filename,
              self.state.filenameInputValue,
              function(response) {
                ...
                <div className="images-edit-content">
                  ...
                  <div className="images-edit-delete-container">
                    ...
                    <div className="images-edit-delete-subtitle">
                      <span>This will remove the image from the server.
                        Before deleting, ensure this image is not being used anywhere.</span>
                    </div>
